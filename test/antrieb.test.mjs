...
test('ohne Befehl sendet der Antrieb den Ruherahmen', () => {
  const { sim, zeit } = simulator();
  sim.takt(zeit());
  assert.equal(sim.protokoll.at(-1).hex, '58 11 40 40 00 00');
});
...
test('ein Fahrbefehl laeuft nach seiner Dauer von selbst aus', () => {
  ...
  sim.fahre('vor', 2, 500);
  sim.takt(zeit());
  assert.equal(sim.protokoll.at(-1).hex, '58 02 40 40 00 00');
...
test('das Protokoll haelt nur Aenderungen fest', () => {
  ...
  assert.equal(sim.gesendet, 3, 'gesendet wird trotzdem in jedem Takt');
});
