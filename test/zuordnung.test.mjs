...
test('jede Richtung ergibt den dokumentierten Rahmen', () => {
  assert.equal(hex(rahmenFuer(fahrt('vor'), voll)), '01 FF FA 00 FF FA 02 00 00');
  assert.equal(hex(rahmenFuer(fahrt('zurueck'), voll)), '00 FF FA 01 FF FA 02 00 00');
  assert.equal(hex(rahmenFuer(fahrt('rechts'), voll)), '01 FF FA 01 FF FA 02 00 00');
  assert.equal(hex(rahmenFuer(fahrt('links'), voll)), '00 FF FA 00 FF FA 02 00 00');
  assert.equal(hex(rahmenFuer(fahrt('vor_rechts'), voll)), '01 FF FA 00 A5 FA 02 00 00');
  assert.equal(hex(rahmenFuer(fahrt('vor_links'), voll)), '01 A5 FA 00 FF FA 02 00 00');
});
...
test('ein vertauschter Drehsinn tauscht die Befehlsbytes', () => {
  ...
  assert.equal(hex(getauscht), '00 FF FA 01 FF FA 02 00 00');
...
test('beim Kurvenfahren bleibt der Drehsinn stimmig', () => {
  ...
  assert.equal(hex(getauscht), '00 FF FA 01 A5 FA 02 00 00');
});
